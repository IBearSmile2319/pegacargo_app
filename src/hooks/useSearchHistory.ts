...
const useSearchHistory = (key: string, idKey: string) => {
    ...
    const clearSearchHistory = () => {
        saveItemToStorage('places', JSON.stringify([]));
        setSearchHistoryItems([]);
    };

    return { searchHistoryItems, addItemToSearchHistory, clearSearchHistory };
};
